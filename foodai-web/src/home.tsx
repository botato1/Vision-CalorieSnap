...
export default function Home() {
  ...
  const getGraphData = (period: string) => {
    ...
    switch (period) {
      ...
      case 'yearly': {
        ...
        const Q_RANGES: [number, number][] = [[1,3],[4,6],[7,9],[10,12]];
  ...
  const renderChatPanel = (mode = 'sidebar') => (
    <div
      className={`
        ml-auto origin-bottom-right overflow-hidden
        bg-white border border-orange-200/80
        shadow-[0_18px_45px_rgba(249,115,22,0.18)]
        transition-[width,height,border-radius,box-shadow,transform,background-color]
        duration-300 ease-out
        ${isChatOpen
          ? `${mode === 'mobile' ? 'w-[calc(100vw-2.5rem)]' : 'w-full'} h-[500px] rounded-3xl flex flex-col scale-100`
          : 'w-16 h-16 rounded-full hover:scale-[1.04] active:scale-[0.98]'}
      `}
    >
      {!isChatOpen ? (
        <button
        ...
        >
          ...
          <span className="sr-only">AI 채팅</span>
        </button>
      ) : (
        <>
          <div className="
          ...
          ">
            <input
              ...
              className="
                ...
                text-slate-900 placeholder:text-slate-400 caret-orange-600
...
{/* 데스크톱 우측 여백 AI 채팅. 버튼은 유지하고, 창은 우측 레일에서 위로 확장됩니다. */}
<div
  className={`
    hidden lg:block fixed bottom-6
    w-[320px] 2xl:w-[360px]
    transition-[transform,width] duration-300 ease-out
    ${isChatOpen ? 'translate-y-0' : 'translate-y-0'}
    right-6
    2xl:right-[max(1.5rem,calc((100vw-80rem)/2-23rem))]
  `}
>
  {renderChatPanel('rail')}
</div>
